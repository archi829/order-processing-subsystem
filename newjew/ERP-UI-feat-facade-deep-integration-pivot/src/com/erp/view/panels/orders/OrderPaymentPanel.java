...
/**
 * Invoice preview + payment capture. Supports partial payments, failed-payment retry.
 */
public class OrderPaymentPanel extends JPanel
    ...
    public OrderPaymentPanel(OrderController controller) {
        ...
        table.getSelectionModel().addListSelectionListener(e -> { if (!e.getValueIsAdjusting()) renderInvoice(); });
        ...
        c.insets = new Insets(4, 6, 4, 6); c.fill = GridBagConstraints.HORIZONTAL; c.weightx = 1; c.anchor = GridBagConstraints.WEST;
        ...
        try { amt = new BigDecimal(payAmount.getText().trim()); }
            ...
            ExceptionHandler.handle(this,
                    new ValidationException(ValidationException.INVALID_REORDER_QUANTITY,
                            "Amount must be numeric.", payAmount));
            return;
        }
        if (amt.signum() <= 0) {
            ExceptionHandler.handle(this,
                    new ValidationException(ValidationException.INVALID_REORDER_QUANTITY,
                            "Amount must be positive.", payAmount));
            return;
        }
    ...
    private void renderInvoice() {
        ...
        invoice.setText(String.format(tpl,
                o.getOrderId(), o.getCustomerName(), o.getCarModel(), o.getChassisType(),
                o.getCarVIN(),
